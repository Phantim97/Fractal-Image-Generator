...
namespace frctl
{
	...
	{
		this->m_width = width;//if errors are given do :m_width(width), m_height(height)
	...
	bool Bitmap::write(std::string filename)
	{
		...
		fileHeader.fileSize = sizeof(frctl::BitmapFileHeader) + sizeof(frctl::BitmapInfoHeader) + m_width*m_height * 3;
		fileHeader.dataOffset = sizeof(frctl::BitmapFileHeader) + sizeof(frctl::BitmapInfoHeader); //starts where pixels do and where info ends
